...
1. `parser::parse_html_to_items` extracts `ItemEntry` records.
1. `tree::ModTree` groups items into modules and renders Rust code.
1. Output is returned to the caller (CLI writes it to disk).
